...
namespace cAlgo.Robots
{
    ...
    public class LRSIEHLERv3 : Robot
    {
        ...
        [Parameter("Momentum Entry", DefaultValue = true)]
        public bool MomentumEntry { get; set; }
        ...
        protected override void OnStart()
        {
            ...
            Sqz = Indicators.GetIndicator<SqueezeMomentum>(BollingerBandsSource, BollingerBandsPeriods, BollingerBandsMultiplier, BollingerBandsMaType, KeltnerMaPeriods, KeltnerMaType, KeltnerAtrMultiplier, KeltnerAtrPeriods, KeltnerAtrMaType, DonchianMidlinePeriods,
            MaSource, MaPeriods, MaType, LinearRegressionPeriods);
        ...
        protected override void OnBar()
        {
            if (OnBar_Bot)
            {
                ...
                //Close position if EHLER momentum changes

                if (!Double.IsNaN(ESAM.Positive.Last(2)) && !Double.IsNaN(ESAM.Negative.Last(1)))
                {
                    foreach (var p in LP)
                    {
                        ClosePosition(p);
                    }
                    //Open a sell when momentum goes red
                    if (MomentumEntry)
                    {
                        if (TrailStop)
                        {
                ...
                if (!Double.IsNaN(ESAM.Negative.Last(2)) && !Double.IsNaN(ESAM.Positive.Last(1)))
                {
                    foreach (var p in SP)
                    {
                        ClosePosition(p);
                ...
                if (Sqz.UpHistogramBars.Last(1) > 0 && LRSI.laguerrersi.HasCrossedAbove(LRSI.oversold, 1))
                {
                ...
                if (Sqz.DownHistogramBars.Last(1) < 0 && LRSI.laguerrersi.HasCrossedBelow(LRSI.overbought, 1))
                {
                    if (SP.Length == 0 && LP.Length == 0 && !Repeater && !Hedging)
                    {
                        if (TrailStop)
        ...
        protected override void OnTick()
        {
            if (!OnBar_Bot)
            {
                ...
                if (LP.Length == 0 && Sqz.UpHistogramBars.Last(1) > 0 && LRSI.laguerrersi.HasCrossedAbove(LRSI.oversold, 1))
                {
                ...
                if (SP.Length == 0 && Sqz.DownHistogramBars.Last(1) < 0 && LRSI.laguerrersi.HasCrossedBelow(LRSI.overbought, 1))
                {
                    if (TrailStop)
                    {
                        ExecuteMarketOrder(TradeType.Sell, SymbolName, Symbol.QuantityToVolumeInUnits(volume), "ESAMRSI", SL, TP, null, true);
