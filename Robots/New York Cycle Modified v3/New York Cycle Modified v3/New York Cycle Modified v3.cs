...
namespace cAlgo.Robots
{
    ...
    public class NewYorkCycleModifiedv3 : Robot
    {
        ...
        protected override void OnBar()
        {
            ...
            var firstSpaceIndex = Bars.OpenTimes.LastValue.ToString().IndexOf(" ");
            var HourTime = Bars.OpenTimes.LastValue.ToString().Substring(firstSpaceIndex + 1);

            if ((HourTime == TradeTime) && TradeState == false)
            {

                Print("HT = " + HourTime);
                Print("It's " + Bars.OpenTimes.LastValue.ToString());
                Print("Trade time is " + TradeTime);
