...
namespace cAlgo.Robots
{
    ...
    public class hendrixmscbot7 : Robot
    {
        ...
        protected override void OnBar()
        {
            ...
            if (Bars.OpenTimes.Last(1).Hour == 11 && Bars.OpenTimes.Last(1).Minute == 50)
            {
                var Buypo = Positions.FindAll("Buy", SymbolName);
                Print("NODE ");
                Print($"NODEcheck  is darkred {isDarkRed()} RedTrigger {!RedTrigger} Symbolspread true {Symbol.Spread < MaxSpread} Checktime true {CheckTime()} Crossover true { CrossOver} " +
               $"emalogic { Bars.ClosePrices.Last(1) > _ema.Result.Last(1) } Abs logic {Math.Abs(GetMinRed()) < GetMaxGreen() } Length true {Buypo.Length == 0}");

                //09/02/2023 11:55:00.000 | is darkred True RedTriggerfalse False Symbolspread true True Checktime true True Crossover true True emalogic True Abs logic True Length true True

                Print($"RSI last 2 {_rsioma.Rsi.Last(2)} Trigger last2 { _rsioma.Trigger.Last(2)} RSI last1 {_rsioma.Rsi.Last(1)}  Trigger last 1 {_rsioma.Trigger.Last(1)}");
                //_rsioma.Rsi.HasCrossedAbove(_rsioma.Trigger, 1)
                if (_rsioma.Rsi.Last(1) > _rsioma.Trigger.Last(1) && _rsioma.Rsi.Last(2) < _rsioma.Trigger.Last(2))
                {
                    Print("NEw cross is true while crossover " + CrossOver);

                    //09/02/2023 11:55:00.000 | RSI last 2 51,57185477094635        Trigger last2 54,980610225864815       RSI last1 59,8569598838673      Trigger last 1 54,99882138200331
                }
            }

            //09/02/2023 11:55:00.000 | is darkred True RedTriggerfalse False Symbolspread true True Checktime true True Crossover true False emalogic True Abs logic True Length true True
