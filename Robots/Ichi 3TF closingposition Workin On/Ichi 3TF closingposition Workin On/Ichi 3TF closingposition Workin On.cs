...
namespace cAlgo.Robots
{
    ...
    public class Ichi3TFclosingpositionWorkinOn : Robot
    {
        ...
        protected override void OnBar()
        {
            ...
            if (isShortPositionOpen != null && Bars.ClosePrices.LastValue > ichimoku15.KijunSen.LastValue)
            {
                ClosePosition();

            }

            if (isLongPositionOpen != null && Bars.ClosePrices.LastValue < ichimoku15.KijunSen.LastValue)
            {
                ClosePosition();
            ...
            if (isShortPositionOpen != null && _ema1.Result.LastValue > _ema2.Result.LastValue)
            {
                ClosePosition();

            }

            if (isLongPositionOpen != null && _ema1.Result.LastValue < _ema2.Result.LastValue)
            {
                ClosePosition();
