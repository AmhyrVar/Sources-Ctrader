...
namespace cAlgo
{
    ...
    public class Ichimoku3timeframes : Robot
    {
        ...
        protected override void OnBar()
        {
            ...
            {
                if (longPositions != null && MarketSeries.Close.Last(1) < ichimoku15.KijunSen.Last(0))
                {
                    ClosePosition(position);
                }
                else if (shortPositions != null && close > ichimoku15.KijunSen.Last(0))
                {
                    ClosePosition(position);
                }
            }
            ...
            if (positionsBuy.Length == 0 && positionsSell.Length == 0)
            {
                ...
                {
                    if (MarketSeries.Close.Last(1) > ichimoku15.SenkouSpanA.Last(27) && MarketSeries.Close.Last(1) > ichimoku15.KijunSen.Last(1) && MarketSeries.Close.Last(1) > ichimoku15.TenkanSen.Last(1) && ichimoku15.SenkouSpanA.Last(1) > ichimoku15.SenkouSpanB.Last(1))
                    {
