...
namespace cAlgo.Robots
{
    ...
    public class ClosingButton : Robot
    {
        ...
        [Parameter("Draw Buy", DefaultValue = true)]
        public bool DB{ get; set; }

        [Parameter("Draw Sell", DefaultValue = false)]
        ...
        protected override void OnStart()
        {
            ...
            {
                if (po.SymbolName == SymbolName && po.TradeType == TradeType.Buy && DB)
                {
                    var a = Chart.DrawHorizontalLine(po.ToString(), po.EntryPrice,GetColor(po),LineThickness) ;
                    a.IsInteractive = true;
                }
                if (po.SymbolName == SymbolName && po.TradeType == TradeType.Sell && DS)
                {
                    var a = Chart.DrawHorizontalLine(po.ToString(), po.EntryPrice, GetColor(po), LineThickness);
                    a.IsInteractive = true;
                }
    ...
    public class TradingPanel : CustomControl
    {
        ...
        static ClosingButton x = new ClosingButton();
        public static Color GetMidCol()
        {
            if (x.DB)
            {
                return Color.Blue;
        ...
        public void Close()
        {
            ...
            var aaa = _robot.Chart.DrawHorizontalLine("midline", (price / trendLines.Length),GetMidCol(), 3);
