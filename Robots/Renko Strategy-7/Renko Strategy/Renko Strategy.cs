...
namespace cAlgo.Robots
{
    ...
    public class RenkoStrategy : Robot
    {
        #region Parameters
        ...
        protected override void OnBar()
        {
            ...
            {
                if (po.Label == "sell1" || po.Label == "buy1")
                    barseeker++;
                if (po.Label == "buy0" && Bars.OpenPrices.Last(1) > Bars.ClosePrices.Last(1) && po.SymbolName == SymbolName)
                    ClosePosition(po);
                if (po.Label == "sell0" && Bars.OpenPrices.Last(1) < Bars.ClosePrices.Last(1) && po.SymbolName == SymbolName)
                    ClosePosition(po);
                if (po.Label == "buy1" && Bars.ClosePrices.Last(1) < _ema.Result.Last(1) && po.SymbolName == SymbolName)
                    ClosePosition(po);
                if (po.Label == "sell1" && Bars.ClosePrices.Last(1) > _ema.Result.Last(1) && po.SymbolName == SymbolName)
                    ClosePosition(po);
            }
        ...
        private void GetTP(Position po)
        {
            ...
            if (po.TradeType == TradeType.Buy)
            {
                ...
                //find the first spike over the EMA
                //def d'un level Bars.ClosePrices.Last(x)>Bars.OpenPrices.Last(x)
                // Math.Max(Close and Buy Last(x-1) < Bars.OpenPrices.Last(x)
                // Math.Max(Close and Buy Last(x+1) < Bars.OpenPrices.Last(x)

                //Bars.ClosePrices.Last(x) > l0_closeprice && > EMA.Last(x)
                //Bars Close and Open Last(x-1 et x+1 > Ema last x+1 et x-1
                // DOOOOOOOOOOONC Close last(x) = tp po

                //X commence à 1 et go ++
