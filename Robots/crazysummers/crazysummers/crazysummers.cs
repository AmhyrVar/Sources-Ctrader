...
namespace cAlgo.Robots
{
    ...
    public class crazysummers : Robot
    {
        ...
        protected override void OnTick()
        {
            ...
            if (Bars.OpenPrices.Last(1)> _ema1.Result.Last(1) && Symbol.Ask <= _ema1.Result.Last(1) && Bpo.Length == 0 && AllowTrade)
            {
                ExecuteMarketOrder(TradeType.Buy, SymbolName, Symbol.QuantityToVolumeInUnits(Lots), "cs",SL,TP);
            }

            if (Bars.OpenPrices.Last(1) < _ema1.Result.Last(1) && Symbol.Bid >= _ema1.Result.Last(1) && Spo.Length == 0 && AllowTrade)
            {
