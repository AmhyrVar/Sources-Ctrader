...
namespace cAlgo.Robots
{
    ...
    public class Ichimokucloudbotv4 : Robot
    {
        ...
        public void WritePend(string DirectionTrade, string TimeframeTrade, double SSA_, double SSB_, double Open_, double Close_)
        {
        ...
        protected override void OnBar()
        {
            ...
            if (TimeCheck() && ichimoku4.SenkouSpanA.LastValue > ichimoku4.SenkouSpanB.LastValue)
            {
                ...
                if ((M30_SSA > M30_SSB && M30_OP > M30_SSB && M30_OP < M30_SSA && M30_CP > M30_SSA) || (M30_SSA < M30_SSB && M30_OP < M30_SSB && M30_OP > M30_SSA && M30_CP > M30_SSB))
                {
                    ...
                    if (pos.Length == 0 && Pending == 0 && SL >= 10)
                    {
                        ...
                        WritePend("Long", "30min", M30_SSA, M30_SSB, M30_OP, M30_CP);
                        Clock_var = 6;
                    }
                ...
                if ((H1_SSA > H1_SSB && H1_OP > H1_SSB && H1_OP < H1_SSA && H1_CP > H1_SSA) || (H1_SSA < H1_SSB && H1_OP < H1_SSB && H1_OP > H1_SSA && H1_CP > H1_SSB))
                {
                    ...
                    if (pos.Length == 0 && Pending == 0 && SL >= 10)
                    {
                        DateTime expiry = Server.Time.AddHours(3);
                        PlaceLimitOrder(TradeType.Buy, SymbolName, Symbol.QuantityToVolumeInUnits(volume), tarprice, "Ichi", SL, 2 * SL, expiry);
                        WritePend("Long", "1H", H1_SSA, H1_SSB, H1_OP, H1_CP);
            ...
            if (TimeCheck() && ichimoku4.SenkouSpanA.LastValue < ichimoku4.SenkouSpanB.LastValue)
            {
                ...
                if ((H1_SSA > H1_SSB && H1_OP > H1_SSB && H1_OP < H1_SSA && H1_CP > H1_SSA) || (H1_SSA < H1_SSB && H1_OP < H1_SSB && H1_OP > H1_SSA && H1_CP > H1_SSB))
                    if ((M30_SSA > M30_SSB && M30_OP > M30_SSB && M30_OP < M30_SSA && M30_CP < M30_SSB) || (M30_SSA < M30_SSB && M30_OP < M30_SSB && M30_OP > M30_SSA && M30_CP < M30_SSA))
                    {
                        ...
                        if (pos.Length == 0 && Pending == 0 && SL >= 10)
                        {
                            ...
                            WritePend("short", "30min", M30_SSA, M30_SSB, M30_OP, M30_CP);
                            Clock_var = 6;
                        }
                ...
                if ((H1_SSA > H1_SSB && H1_OP > H1_SSB && H1_OP < H1_SSA && H1_CP < H1_SSB) || (H1_SSA < H1_SSB && H1_OP < H1_SSB && H1_OP > H1_SSA && H1_CP < H1_SSA))
                {
                    ...
                    if (pos.Length == 0 && Pending == 0 && SL >= 10)
                    {
                        DateTime expiry = Server.Time.AddHours(3);
                        PlaceLimitOrder(TradeType.Sell, SymbolName, Symbol.QuantityToVolumeInUnits(volume), tarprice, "Ichi", SL, 2 * SL, expiry);
                        WritePend("short", "1H", H1_SSA, H1_SSB, H1_OP, H1_CP);
                        h1_trade = true;
                        Clock_var = 6;
                    }
