...
namespace cAlgo.Robots
{
    ...
    public class RenkoStrategy7v30222 : Robot
    {
        #region Parameters
     ...
     // On a l'entr�e en Range trigger par la variable RangeState
           ...
           if (ZTriggers.Count >= 2 && !RangeState )
            ...
            if (RangeState && TimeCheck())
            {
                ...
                if (EightOverEma() && Positions.Count(p => p.SymbolName == SymbolName && p.Label.Contains("buy") && p.TradeType == TradeType.Buy) == 0)
                {
                    ...
                    var label = "buy";
                    var slrange = 16 * Math.Abs(Bars.OpenPrices.Last(1) - Bars.ClosePrices.Last(1)) / Symbol.PipSize;
                    ExecuteMarketOrderAsync(TradeType.Buy, SymbolName, Symbol.QuantityToVolumeInUnits(Volume), label, slrange, null);
                    Rangeanchor = Symbol.Ask;
                    var salvageentry = Bars.OpenPrices.Last(1) - (2 * (Bars.ClosePrices.Last(1) - Bars.OpenPrices.Last(1)) );
                    var slinpips =10*Math.Abs(Bars.OpenPrices.Last(1) - Bars.ClosePrices.Last(1)) / Symbol.PipSize;
                    PlaceStopOrder(TradeType.Sell, SymbolName, Symbol.QuantityToVolumeInUnits(Volume), salvageentry, "stop",slinpips,null);
                ...
                if (EightUnderEma() && Positions.Count(p => p.SymbolName == SymbolName && p.Label.Contains("sell") && p.TradeType == TradeType.Sell) == 0)
                {
                    ...
                    var slrange = 16 * Math.Abs(Bars.OpenPrices.Last(1) - Bars.ClosePrices.Last(1)) / Symbol.PipSize;
                    ExecuteMarketOrderAsync(TradeType.Sell, SymbolName, Symbol.QuantityToVolumeInUnits(Volume), label,slrange,null);
                    Rangeanchor = Symbol.Bid;
                    var salvageentry = Bars.OpenPrices.Last(1) + (2 * (Bars.OpenPrices.Last(1) -Bars.ClosePrices.Last(1) ));
                    var slinpips = 10*Math.Abs(Bars.OpenPrices.Last(1) - Bars.ClosePrices.Last(1)) / Symbol.PipSize;
                    PlaceStopOrder(TradeType.Buy, SymbolName, Symbol.QuantityToVolumeInUnits(Volume), salvageentry, "stop",slinpips,null);
                    barstop = 0;
                }
            }
